...
for row in teeth_brushing_csv:
    ...
    if len(old_file_name) > 0 and os.path.isfile(old_file_name[0]):
        # file is inside folder, so need to rename and put into subject folder
        print("old file found")

        # create subject folder if it doesn't already exist
        if os.path.exists(path_to_videos + subject) == False:
            # subject folder does not exist, need to create one
            os.makedirs(path_to_videos + subject)

        # recorded_date can be either 6 or 7 characters long depending on if the
        # day in month is greater than 9
        ...
        if recorded_date[4] == "/":
            ...
            if int(recorded_date[-5:-3]) < 12:
                ...
                # from the day
                date_minus_1 = datetime(
                    int(recorded_date[5:7]) + 2000,
                    int(recorded_date[0]),
                    int(recorded_date[2:4]),
                ) - timedelta(days=1)
            ...
            if int(recorded_date[-5:-3]) < 12:
                ...
                new_file_name = (
                    ...
                    + subject
                    + "-"
                    + recorded_date.replace("/", "-")[:6]
                    + ".mp4"
                )

        os.rename(old_file_name[0], path_to_videos + "/" + subject + new_file_name)
    else:
