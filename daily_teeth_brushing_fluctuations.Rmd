...
daily_fluctuations<-filter(daily_fluctuations, subject!="115", serious_other_factors=="0", !is.na(time_brushing))
...
# has unusually high child mood and encouragement
overall_subject<-filter(overall_subject, subject!="101")
...
```{r,warning=FALSE,fig.width=10, fig.height=8,echo = FALSE}
# plot encouragement vs. child mood
between_subject_encouragement_vs_child_mood <- ggplot(data=overall_subject, mapping=aes(x=mean_child_mood, y=mean_encouragement))
between_subject_encouragement_vs_child_mood +
  stat_cor(method = "pearson",  size = 6) +
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE) +
  geom_point(shape=1) +
