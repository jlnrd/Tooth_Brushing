...
mean_parent_education <- (subject_background_information$parent1_edu + subject_background_information$parent2_edu) / 2

# subject 107 only provided parent education for parent 1, so making average education equal to parent 1 edu
mean_parent_education <- replace(mean_parent_education, 7, 18)

# add the subject_background information into the overall_subject data.frame
overall_subject <- cbind(overall_subject, subject_background_information %>% dplyr::select(pss_sum, income_median), mean_parent_education)
```
